//#region node_modules/.pnpm/zone.js@0.16.3/node_modules/zone.js/fesm2015/zone.js
/**
* @license Angular
...
function __symbol__(name) {
	...
	return (typeof rawPrefix === "string" ? rawPrefix : "__zone_symbol__") + name;
}
function initZone() {
	...
	const _ZoneImpl = class _ZoneImpl {
		...
		getZoneWith(key) {
			let current = this;
			while (current) {
				if (Object.hasOwn(current._properties, key)) return current;
...
function patchProperty(obj, prop, prototype) {
	...
	if (Object.hasOwn(obj, onPropPatchedSymbol) && obj[onPropPatchedSymbol]) return;
...
function patchClass(className) {
	...
	for (prop in OriginalClass) if (prop !== "prototype" && Object.hasOwn(OriginalClass, prop)) _global[className][prop] = OriginalClass[prop];
}
...
function patchMethod(target, name, patchFn) {
	let proto = target;
	while (proto && !Object.hasOwn(proto, name)) proto = ObjectGetPrototypeOf(proto);
...
var KNOWN_EVENT_LISTENER_OPTIONS = [
	"capture",
	"once",
	"passive",
	"signal"
...
function patchEventTarget(_global2, api, apis, patchOptions) {
	...
	function patchEventTargetMethods(obj, patchOptions2) {
		...
		let proto = obj;
		while (proto && !Object.hasOwn(proto, ADD_EVENT_LISTENER)) proto = ObjectGetPrototypeOf(proto);
		...
		function copyEventListenerOptions(options) {
			...
			for (const key of KNOWN_EVENT_LISTENER_OPTIONS) if (!Object.hasOwn(newOptions, key) && key in options) newOptions[key] = options[key];
			return newOptions;
		}
	...
	Zone3.__load_patch("ZoneAwarePromise", (global2, Zone4, api) => {
		...
		api.onUnhandledError = (e) => {
			if (api.showUncaughtError()) {
				...
				if (rejection && e.zone && e.task) console.error("Unhandled Promise rejection:", rejection instanceof Error ? rejection.message : rejection, "; Zone:", e.zone.name, "; Task:", e.task && e.task.source, "; Value:", rejection, rejection instanceof Error ? rejection.stack : void 0);
